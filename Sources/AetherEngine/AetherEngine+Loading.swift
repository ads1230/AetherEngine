...
extension AetherEngine {
    ...
    func loadSoftware(
        ...
        #if os(iOS) || os(tvOS)
        // Same for a load while the app is BACKGROUNDED (lock-screen play
        // command re-tuning a parked session): didEnterBackground already
        // fired, so the shed decision must be re-made here or the fresh
        // session runs the full video pipeline with no GPU access;
        // didBecomeActive un-sheds and PiP's rising edge handles PiP.
        if isBackgrounded && !pictureInPictureActive {
        ...
        // SW-PiP Phase C: mirror the published cues (primary + secondary + DVB page) into the
        // renderer's frame compositor; the PiP flag gates actual drawing (fullscreen stays
        // host-overlay-only). The page rides along so PiP burn-in gets the same engine-owned
        // page state as the overlays, instead of window-filtering re-send bookkeeping.
        Publishers.CombineLatest3($subtitleCues, $secondarySubtitleCues, $dvbSubtitlePage)
            .sink { [weak self, weak host] _, _, _ in
                guard let self, let host else { return }
                host.updateSubtitleCompositor(cues: self.subtitleCompositorFeedCues,
